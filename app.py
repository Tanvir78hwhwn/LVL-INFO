...
# ===== FIX 1: format_num Function Added =====
def format_num(num):
    try:
        return f"{int(num):,}"
    except:
...
def calculate_level_progress(current_exp, current_level):
    try:
        ...
        # ===== FIX 2: Negative Protection Added =====
        exp_needed = max(0, exp_for_next - current_exp)
        exp_needed_for_100 = max(0, exp_for_100 - current_exp)
        ...
        if exp_range_for_level > 0:
            progress_percentage = min(
                100,
                max(0, (exp_in_current_level / exp_range_for_level) * 100)
            )
        else:
...
def fetch_player_info(uid):
    try:
        api_url = f"https://najmi-info-all-server.vercel.app/player-info?uid={uid}"
...
def home():
    return jsonify({
        ...
        "endpoints": {
            ...
            "/level/<level_number>/exp": "Get EXP for specific level"
        }
    })
