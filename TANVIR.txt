...
def get_level_info(uid):
    try:
        ...
        if not account_info:
            return jsonify({
                "success": False,
                "message": "AccountInfo not found"
            })

        nickname = account_info.get("AccountName", "Unknown")
        current_level = int(account_info.get("AccountLevel", 0))
        current_exp = int(account_info.get("AccountEXP", 0))
        ...
        if not progress:
            return jsonify({
                "success": False,
                "message": "Level calculation failed",
                "debug_level": current_level,
                "debug_exp": current_exp
            })
